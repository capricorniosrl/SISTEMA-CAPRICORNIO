...
$sql2 = $pdo->prepare("SELECT  COUNT(*) as cuotas FROM tb_semicontado semi INNER JOIN tb_comprador comp INNER JOIN tb_cuotas cu WHERE ((semi.id_comprador_fk=comp.id_comprador) AND comp.id_comprador='$id') AND cu.id_semicontado_fk='$id_semicontado'");
...
$pdf->SetFillColor(233, 233, 233); // Establecer color de fondo (RGB)
$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(120, 8, ' DETALLES - SEMI CONTADO', 0, 1, 'L', true); //
$pdf->SetTextColor(0, 0, 0);
...
$pdf->SetFont('Arial', '', 10);
$pdf->Cell(0, $interlineado, "9 DE AGOSTO DEL 2024", 0, 1, 'L');
$pdf->SetFont('Arial', 'B', 10);
...
$pdf->Cell(22, 7, utf8_decode("CUOTAS"), 0, 0, 'C', true);
$pdf->Cell(1, 7, utf8_decode(""), 0, 0, 'C');
$pdf->Cell(40, 7, utf8_decode("FECHA"), 0, 0, 'C', true);
$pdf->Cell(1, 7, utf8_decode(""), 0, 0, 'C');
$pdf->Cell(42, 7, utf8_decode("CUOTA INICIAL"), 0, 0, 'C', true);
$pdf->Cell(1, 7, utf8_decode(""), 0, 0, 'C');
...
$pdf->Cell(22, 7, utf8_decode("INICIAL"), "B", 0, 'C', true);
$pdf->Cell(1, 7, utf8_decode(""), "B", 0, 'C');
$pdf->Cell(40, 7, utf8_decode(date("d - m - Y", strtotime($datos['fecha_registro']))), "B", 0, 'L', true);
$pdf->Cell(1, 7, utf8_decode(""), "B", 0, 'C');
$pdf->Cell(42, 7, utf8_decode($datos['cuota_inicial'] . ' Dólares'), "B", 0, 'R', true);
$pdf->Cell(1, 7, utf8_decode(""), "B", 0, 'C');
$pdf->Cell(0, 7, utf8_decode($monto_cancelar . " Dólares"), "B", 0, 'R', true);
$pdf->Cell(1, 7, utf8_decode(""), "B", 1, 'C');
$pdf->Ln(1);
...
$sql3 = $pdo->prepare("SELECT cu.numero_recibo, cu.fecha_registro_pago, cu.id_cuota, cu.nombre_cuota, cu.monto FROM tb_semicontado semi INNER JOIN tb_comprador comp INNER JOIN tb_cuotas cu WHERE ((semi.id_comprador_fk=comp.id_comprador) AND comp.id_comprador='$id') AND cu.id_semicontado_fk='$id_semicontado'");
$sql3->execute();
$contador = 0;
...
foreach ($dato3 as $valor) {
    ...
    $pdf->Cell(22, 7, utf8_decode($valor['nombre_cuota']), "B", 0, 'C', true);
    $pdf->Cell(1, 7, utf8_decode(""), "B", 0, 'C');
    $pdf->Cell(40, 7, utf8_decode($fechaFormateada), "B", 0, 'K', true);
    $pdf->Cell(1, 7, utf8_decode(""), "B", 0, 'C');

    if ($valor['monto']) {
        $pdf->Cell(42, 7, utf8_decode($valor['monto'] . " Dólares"), "B", 0, 'R', true);
        $pdf->Cell(1, 7, utf8_decode(""), "B", 0, 'C');
    } else {
        $pdf->Cell(42, 7, utf8_decode("SIN CANCELAR"), "B", 0, 'R', true);
        $pdf->Cell(1, 7, utf8_decode(""), "B", 0, 'C');
    }
...
$pdf->Cell(63, 7, utf8_decode("TOTAL"), "B", 0, 'C', true);
$pdf->Cell(1, 7, utf8_decode(""), "B", 0, 'C');
