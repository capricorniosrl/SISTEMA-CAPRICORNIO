...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $sql_buscar = $pdo->prepare("SELECT info.fecha_cierre, info.fecha_registro, info.monto, info.lote, info.manzano, con.celular, us.id_usuario, ag.detalle_agenda, ag.visitantes, us.nombre as nom_user, us.ap_paterno, us.ap_materno, UPPER(cli.nombres) as nombre_usuario, UPPER(cli.apellidos) as apellidos, cli.tipo_urbanizacion, ag.fecha_visita, UPPER(cli.detalle) as detalle, ag.asistio FROM tb_clientes cli INNER JOIN tb_agendas ag INNER JOIN tb_contactos con INNER JOIN tb_usuarios us INNER JOIN tb_informe info WHERE (cli.tipo_urbanizacion='$nombre_urbanizacion' AND ((( ag.id_cliente_fk=cli.id_cliente) AND ag.id_usuario_fk = us.id_usuario) AND ag.estado=1) AND (ag.id_agenda=info.id_agenda_fk AND info.monto>0) AND con.id_contacto=cli.id_contacto_fk);");
