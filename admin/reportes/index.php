...
$stmt = $pdo->prepare("SELECT us.nombre, us.ap_paterno, us.ap_materno, inf.tipo_cliente, inf.detalle_tipo_cliente, inf.fecha_registro, inf.monto, inf.tipo_pago, inf.num_recibo, inf.num_transferencia, cli.tipo_urbanizacion, inf.resumen_visita, inf.seguiente_paso FROM tb_agendas ag INNER JOIN tb_usuarios us INNER JOIN tb_informe inf INNER JOIN tb_clientes cli WHERE ((((ag.id_agenda = $id AND ag.id_usuario_fk = us.id_usuario) AND ag.id_cliente_fk = cli.id_cliente ) AND us.id_usuario = $id_usuario)AND ag.id_cliente_fk = cli.id_cliente) AND ag.id_agenda=inf.id_agenda_fk");
...
$pdf->Cell(0,5,'',0,1,'C');
   ...
   $pdf->Cell(3,5,'',0,0,'C');
    ...
    $pdf->SetFont('Arial','',11);        
    // fin de la fuente negro
   $pdf->MultiCell(185,4.5,$datos['seguiente_paso'],0,"J");
   //Inicio de feunte titulo
   $pdf->SetTextColor(5,5,123);
   $pdf->SetFont('Arial','B',9);
   //Fin de feunte titulo
